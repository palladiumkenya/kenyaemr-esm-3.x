...
type BillsBreakDownTableProps = {
  breakDownType?: string;
};

const BillsBreakDownTable: React.FC<BillsBreakDownTableProps> = ({ breakDownType = '' }) => {
  const { t } = useTranslation();
  const id = useId();
  const config = useConfig();
  const layout = useLayoutType();
  const responsiveSize = isDesktop(layout) ? 'sm' : 'lg';
  const [billPaymentStatus, setBillPaymentStatus] = useState(breakDownType);
  ...
  const headerData = [
    {
      header: t('visitTime', 'DateTime'),
      key: 'visitTime',
    },
    {
      header: t('identifier', 'BillNo'),
      key: 'identifier',
    },
    {
      header: t('name', 'Name'),
...
export default BillsBreakDownTable;
