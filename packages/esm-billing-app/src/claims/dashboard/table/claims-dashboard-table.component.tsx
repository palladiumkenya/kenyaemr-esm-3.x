...
import styles from './claims-dashboard-table.scss';
...
type ClaimsDashboardTableProps = {
  defaultBillPaymentStatus?: string;
};

const ClaimsDashboardTable: React.FC<ClaimsDashboardTableProps> = ({ defaultBillPaymentStatus = '' }) => {
  ...
  const claimsUrls = '${openmrsSpaBase}/home/billing/patient/${patientUuid}/${billUuid}/claims?claimUuid=${claimUuid}';
  ...
  const rowData = results?.map((bill, index) => ({
    ...
    patientName: (
      <ConfigurableLink
        ...
        to={claimsUrls}
        templateParams={{ patientUuid: bill.patientUuid, billUuid: bill.uuid, claimUuid: bill.uuid }}>
        {bill.patientName}
      </ConfigurableLink>
    ),
      ...
      {bills?.length > 0 ? (
        <div className={styles.claimsListContainer}>
          ...
          <DataTable
            ...
            {({ rows, headers, getRowProps, getTableProps }) => (
              <TableContainer>
                <Table {...getTableProps()} aria-label="claims list">
                  <TableHead>
                    <TableRow>
                      {headers.map((header) => (
              ...
              <Layer level={0}>
                <Tile className={styles.filterEmptyStateTile}>
                  <p className={styles.filterEmptyStateContent}>
                    {t('noMatchingClaimsToDisplay', 'No matching claims to display')}
                  </p>
        ...
        <Layer className={styles.emptyStateContainer}>
          <Tile className={styles.tile}>
            ...
            <p className={styles.content}>{t('noClaimsToDisplay', 'There are no claims to display.')}</p>
          </Tile>
        </Layer>
      )}
    </>
...
export default ClaimsDashboardTable;
