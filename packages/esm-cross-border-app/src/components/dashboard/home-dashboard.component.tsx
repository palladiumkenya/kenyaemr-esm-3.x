...
export default function Dashboard() {
  ...
  const ungroupedDashboards =
    extensionStore.slots['cross-border-page-dashboard-slot']?.assignedExtensions
      .map((e) => e.meta)
    ...
    <>
      <div className={styles.homePageWrapper}>
        <section className={isDesktop(layout) ? styles.dashboardContainer : styles.dashboardContainerTablet}>
          {isDesktop(layout) && <ExtensionSlot name="cross-border-side-nav-slot" key={layout} />}
          <DashboardView title={activeDashboard?.name} dashboardSlot={activeDashboard?.slot} />
        </section>
        <WorkspaceContainer overlay contextKey="cross-border" />
      </div>
    </>
  );
}
