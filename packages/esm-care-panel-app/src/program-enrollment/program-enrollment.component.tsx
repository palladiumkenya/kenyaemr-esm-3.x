...
const ProgramEnrollment: React.FC<ProgramEnrollmentProps> = ({
  ...
  const shouldHideHeading = data.length === 0 || data.some((program) => program.programName === programName);
      ...
      {shouldHideHeading && (
        <div className={isTablet ? styles.tabletHeading : styles.desktopHeading}>
          <h4 className={styles.title}>
            {' '}
            {status == true
              ? t('currentEnrollmentDetails', 'Current enrollment details')
              : t('historicEnrollmentDetails', 'Historic enrollment details')}
          </h4>
        </div>
      )}

      {data?.map((program) =>
        program.active === true && program.programName === programName ? (
          <>
        ...
        ) : program.active === false && program.programName === programName ? (
          <>
            <Tile className={styles.card}>
              <div className={styles.labelContainer}>
                <div className={styles.content}>
                ...
                {program.programName === ProgramType.HIV && program ? (
                  <>
                    <div className={styles.content}>
                      <p>{t('whoStage', 'WHO Stage')} </p>
