...
export const ReferredTests: React.FC = () => {
      ...
      .map((entry) => ({
        ...
        actions: (
          <OverflowMenu flipped={true}>
            <OverflowMenuItem itemText="Pick Request" onClick={() => 'Pick Request'} />
            <OverflowMenuItem itemText="Rejected Order" onClick={() => 'Rejected Order'} />
          </OverflowMenu>
        ),
      }));
  }, [paginatedResults]);
  ...
  return isLoading ? (
    ...
    <div>
      <DataTable rows={rows} headers={tableColumns} useZebraStyles overflowMenuOnHover={true}>
                  ...
                  {rows.map((row) => (
                    <React.Fragment key={row.id}>
                      ...
                      {expandedRows.has(row.id) && (
                        <TableRow>
                          <TableCell colSpan={tableColumns.length + 1}></TableCell>
                        </TableRow>
                      )}
                    </React.Fragment>
                  ))}
