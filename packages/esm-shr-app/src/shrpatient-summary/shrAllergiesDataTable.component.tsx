...
const SHRAllergiesDataTable: React.FC<SHRDataTableProps> = ({ data }) => {
  const { t } = useTranslation();
  const tableHeaders = ['Allergen', 'Reaction', 'Severity', 'Onset Date', 'Date Recorded'];
            ...
            <TableBody>
              {tableRows.map((row) => (
                <TableRow {...getRowProps({ row })}>
                  <TableCell>{row[0].allergen}</TableCell>
                  <TableCell>{row[0].reaction}</TableCell>
                  <TableCell>{row[0].severity}</TableCell>
                  <TableCell>{row[0].onsetDate}</TableCell>
...
export default SHRAllergiesDataTable;
